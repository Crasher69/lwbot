...
//НАСТРОЙКИ
//Прогулка :
var walk2='foret';//если горы то 'montagne'
var walk1 = 'montagne';

//Навыки профиля ( осторожней при смене,не потеряйте ковычки и точку с запятой,иначе встанет)
var train1 = 'vitesse';
...
//Паузы, короткую не рогаем, остальные настраиваем под ваши возможности ,смотрим,чтобы первая быламеньше второй- таккак берет рандомную паузу из промежутка между ними
var shortPause1 = 50;
var shortPause2 = 75;
var mediumPause1 =600;
var mediumPause2 = 700;
var longPause1 = 1000;
var longPause2 = 1200;

//Навык .по которому будет смотреть- прокачено ли,вместо  galop,если его нет в вашем профиле 1 или 2 навыком, вставляем любой из зажирняемых навыков вашего профиля
...
function GeneticsTraining(){
 ...
 EquipAmunition();
   ...
   else if (!compComplet)
    {
        HayToGive = -1;
        OatsToGive = 10;   
      if (chevalEnergie>20 && timeConvert() < 1270) { // 1360
              ...
              //ORProg();
			   ...
			   setTimeout(OR,1100);
 	...
 	else if (chevalAge<120)
				{
					...
					HayToGive = 12;
					OatsToGive = 14;
					if (enduranceComplet==true && dressageComplet==true && sautComplet==true)
						{
							setTimeout(function() {ORProg();}, 400); 
						}
					else
					 setTimeout(function() {Walks(walk2);}, 400); 
				}	
 ...
 else 
    {
        if (chevalSexe == "feminin"){
			...
			console.log('Offers in Cookie: '+parseInt(readCookie(offers),10));
            GiveOfferings(5);
...
function BirthProg(){
 ...
 function Train1() //докачка треней на родах
    {
        var d=document.getElementsByClassName('action action-style-4 competition-trail-class action-disabled');
        if (d[0] !== undefined)
       {
           ORProg();
        } 
...
function doReproduction(){ 
  ...
  document.getElementById('race').options[6].selected = true;
  ...
  if (document.getElementById('race').options[6].selected = true){    
 d.item(0).click();     
}
}
